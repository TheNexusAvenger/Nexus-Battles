...
local TYPE_TO_PATH = {
    ReplicatedContainer = "Common.Object.Base.ReplicatedContainer",
    ReplicatedTable = "Common.Object.Base.ReplicatedTable",
    Timer = "Common.Object.Round.Timer",
}
...
local ObjectReplication = NexusRoundSystem:GetResource("NexusInstance.NexusInstance"):Extend()
...
function ObjectReplication:GetClass(Type)
    ...
    if not self.TypeClasses[Type] then
        ...
        if Path then
            NexusRoundSystem:GetResource(Path)
        end
    end
